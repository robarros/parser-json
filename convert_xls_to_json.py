...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("  python3 convert_xls_to_json.py contas.xls fv4")
        print("  python3 convert_xls_to_json.py contas.xls fv4 resultado.json")
        print("  python3 convert_xls_to_json.py dados.xlsx ei7")
        print("")
        print("Parâmetros:")
    ...
    if not os.path.exists(xls_file):
        ...
        if not xls_file.lower().endswith(('.xls', '.xlsx', '.xlsm', '.xlsb')):
            ...
            print(f"")
            print(f"📋 Exemplos:")
            print(f"   python3 convert_xls_to_json.py contas.xls {xls_file}")
            print(f"   python3 convert_xls_to_json.py dados.xlsx {xls_file}")
        else:
            print(f"❌ Erro: Arquivo '{xls_file}' não encontrado!")
            print(f"   Verifique se o caminho está correto.")
        sys.exit(1)
